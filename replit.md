...
- Design tokens: `artifacts/next-leap/src/index.css` is the single source. `:root` holds raw un-prefixed literals (always emitted, safe for `var()` in inline styles and SVG attrs); `@theme inline` holds only `--color-*` → `var(--raw)` mappings; plain `@theme` holds type/radius/shadow. `pnpm --filter @workspace/next-leap run check:tokens --strict` enforces it. Fonts are self-hosted via `pnpm run fonts`; icons via `pnpm run icons` (Chromium rasterises `assets-src/*.svg`).
...
- Portfolio Pulse is dark and confident; its daily podcast-style audio brief is a core feature, not a nice-to-have.
- **The product is named Shortlist; the URL path is still `/next-leap/`.** Deliberate: no accounts means the link is the account, so moving the path would 404 every saved and shared board. The manifest `id` is pinned to `/next-leap/` forever, even if the path later moves. Shortlist is light — "dark mode is for nerds".
